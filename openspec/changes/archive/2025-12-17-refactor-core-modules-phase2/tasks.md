...
- [x] 7.1 更新 examples/
  - [x] 7.1.1 创建 examples/rustfs/bucketops-new.go 使用新 API
  - [x] 7.1.2 创建 examples/rustfs/objectops-new.go 使用新 API
  - [x] 7.1.3 创建 examples/rustfs/multipart-new.go 分片上传示例
  - [x] 7.1.4 保留旧示例以保持向后兼容

- [x] 7.2 更新 README.md
  - [x] 7.2.1 添加新 API 使用示例
  - [x] 7.2.2 更新快速开始部分
  - [x] 7.2.3 添加新旧 API 对比（使用折叠部分）
  - [x] 7.2.4 更新示例索引和运行说明

## 8. 测试验证

- [x] 8.1 单元测试
  - [x] 8.1.1 运行所有单元测试确保通过（全部通过）
  - [x] 8.1.2 确保测试覆盖率 > 60%（已达标）

- [x] 8.2 集成测试
  - [x] 8.2.1 测试与真实 RustFS 服务器的交互（示例运行成功）
  - [x] 8.2.2 测试向后兼容性（测试通过）

- [x] 8.3 构建验证
  - [x] 8.3.1 运行 `go build ./...` 确保编译通过
  - [x] 8.3.2 运行 `go mod tidy` 确保依赖正确
