...
- [x] 6.1.1 所有新功能单元测试（目标覆盖率 >80%）
  ...
  - [x] go --% test -coverprofile=coverage.out ./pkg/sse ./pkg/objectlock ./pkg/restore ./types ./pkg/cors
  - [x] go --% tool cover -func=coverage.out (total coverage 85.5%)
- [x] 6.1.2 边界条件测试
...
- [x] 6.2.3 端到端场景测试
  - [x] examples/s3/e2e-scenario.go (bucket config + object lifecycle)

### 6.3 性能测试
- [x] 6.3.1 加密性能测试
  - [x] examples/s3/perf-sse.go
- [x] 6.3.2 大文件操作性能测试
  - [x] examples/s3/perf-large-object.go
- [x] 6.3.3 并发操作测试
  - [x] examples/s3/perf-concurrent.go

### 6.4 代码质量
- [x] 6.4.1 代码审查
- [x] 6.4.2 静态分析（golangci-lint）
